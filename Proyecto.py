...
def poissonFunc(x, lambda_val):
  return -(math.log(1 - x) / lambda_val)

def DistExp(lambda_val):
    return  - float((1/lambda_val)*math.log(1-random.random()))
...
class Cajero:
  ...
  def despacho_cliente(self, cliente):
    tiempo_despacho = DistExp(self.CLIENTE_X_HORA)
...
def setup(env, Cajero):
  ...
  while True:
    CLIENTES_interval = poissonFunc(random.random(), TOTAL_CLIENTES_HORA)
...
print("Numero de clientes en la cola por hora: ", (1 / (tiempoEspera / CLIENTES)))
